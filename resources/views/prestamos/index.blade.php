                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="{{ route('prestamos.create') }}" class="btn btn-info"><i class="fa fa-plus"></i> Nuevo
                                Préstamo</a>
                            <a href="{{ route('prestamos.create_solicitud') }}" class="btn btn-danger"><i
                                    class="fa fa-plus"></i> Solicitudes Portal</a>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                                    ...
                                    @foreach ($prestamos as $prestamo)
                                        @php
                                            ...
                                            if ($prestamo->descripcion == 'PRESTAMO') {
                                                if (
                                                    date('Y-m-d') >
                                                    date('Y-m-d', strtotime($prestamo->fecha_devolucion))
                                                ) {
                                                    $rojo = 'bg-red';
                                                }
                                            }
                                        ...
                                        <tr class="{{ $rojo }}">
                                            ...
                                            <td class="btns-opciones">
                                                @if ($prestamo->descripcion == 'PRESTAMO')
                                                    <a href="{{ route('prestamos.comprobantePrestamo', $prestamo->id) }}"
                                                        class="evaluar" target="_blank"><i class="fa fa-file-pdf"
                                                            data-toggle="tooltip" data-placement="left"
                                                            title="Boleta Préstamo"></i></a>
                                                @else
                                                    @if ($prestamo->descripcion == 'DEVOLUCION')
                                                        <a href="{{ route('prestamos.comprobanteDevolucion', $prestamo->id) }}"
                                                            class="evaluar" target="_blank"><i class="fa fa-file-pdf"
                                                                data-toggle="tooltip" data-placement="left"
                                                                title="Boleta Devolución"></i></a>
                                                    @endif
                                                @endif

                                                @if ($prestamo->tipo == 'EGRESO' && $prestamo->estado == 1)
                                                    <a href="#"
                                                        data-url="{{ route('prestamos.registra_devolucion', $prestamo->id) }}"
                                                        data-id="{{ $prestamo->id }}" class="ir-evaluacion"><i
                                                            class="fa fa-clipboard-check" data-toggle="tooltip"
                                                            data-placement="left" title="Devolución"></i></a>
                                                @endif

                                                @if ($prestamo->solicitud)
                                                    <a href="{{ route('prestamos.edit_solicitud', $prestamo->id) }}"
                                                ...
                                                @if (Auth::user()->tipo == 'ADMINISTRADOR')
                                                    <a href="#"
                                                        data-url="{{ route('prestamos.destroy', $prestamo->id) }}"
